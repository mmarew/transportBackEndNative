...
const {
  USER_BALANCE_ENDPOINTS,
} = require("../EndPoints/userBalance.endpoints");
const {
  USER_BALANCE_TRANSFER_ENDPOINTS,
} = require("../EndPoints/userBalanceTransfer.endpoints");
const {
  USER_DEPOSIT_ENDPOINTS,
} = require("../EndPoints/userDeposit.endpoints");
const { USER_REFUND_ENDPOINTS } = require("../EndPoints/userRefund.endpoints");
const {
  USER_SUBSCRIPTION_ENDPOINTS,
} = require("../EndPoints/userSubscription.endpoints");
const { COMMISSION_ENDPOINTS } = require("../EndPoints/commission.endpoints");
const {
  COMMISSION_RATES_ENDPOINTS,
} = require("../EndPoints/commissionRates.endpoints");
const {
  COMMISSION_STATUS_ENDPOINTS,
} = require("../EndPoints/commissionStatus.endpoints");
const {
  DRIVER_EARNING_ENDPOINTS,
} = require("../EndPoints/driverEarning.endpoints");
const {
  PAYMENT_METHOD_ENDPOINTS,
} = require("../EndPoints/paymentMethod.endpoints");
const {
  PAYMENT_STATUS_ENDPOINTS,
} = require("../EndPoints/paymentStatus.endpoints");
const { PAYMENTS_ENDPOINTS } = require("../EndPoints/payments.endpoints");
const {
  JOURNEY_PAYMENTS_ENDPOINTS,
} = require("../EndPoints/journeyPayments.endpoints");
const {
  SUBSCRIPTION_PLAN_ENDPOINTS,
} = require("../EndPoints/subscriptionPlan.endpoints");
const {
  SUBSCRIPTION_PLAN_PRICING_ENDPOINTS,
} = require("../EndPoints/subscriptionPlanPricing.endpoints");
const {
  DEPOSIT_SOURCE_ENDPOINTS,
} = require("../EndPoints/depositSource.endpoints");
const {
  FINANCIAL_INSTITUTION_ACCOUNT_ENDPOINTS,
} = require("../EndPoints/financialInstitutionAccount.endpoints");
...
router.use(
  SUBSCRIPTION_PLAN_PRICING_ENDPOINTS.MOUNT,
  subscriptionPlanPricingRoutes,
);

// Financial institutions and sources
router.use(DEPOSIT_SOURCE_ENDPOINTS.MOUNT, depositSourceRoutes);
router.use(
  FINANCIAL_INSTITUTION_ACCOUNT_ENDPOINTS.MOUNT,
  financialInstitutionAccountRoutes,
);
