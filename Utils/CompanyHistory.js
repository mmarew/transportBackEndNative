"use strict";
/**
 * CompanyHistory utility
 * ───────────────────────
 * Single source of truth for all company change history.
 * Replaces the two previous utilities (CompanyStatusHistory + CompanyProfileHistory).
 *
 * Every change — whether a status transition or a profile field update — writes
 * one or more rows to CompanyHistory. The table is append-only (never updated/deleted).
 *
...
exports.getHistory = async (companyUniqueId, { page = 1, limit = 20, fieldName, source } = {}) => {
  ...
  const [[{ total }]] = await exec().query(
    `SELECT COUNT(*) AS total FROM CompanyHistory h ${whereClause}`, params,
  );
