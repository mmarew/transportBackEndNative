...
**Current state:** 127 ✅ passed · 28 ⏩ skipped · 0 ❌ failed (155 total)
...
| 2 | `POST /api/finance/userDeposit/initiateSantimPay` | 400 `VALIDATION_ERROR` | Body validation schema expects different field names than the test sends |
| 3 | `POST /api/driver/takeFromStreet` | 500 `isOperational:true` | Unhandled error in the controller when processing take-from-street — may need to check the route handler |
| 4 | `DELETE /api/driver/request/:id` | 500 `isOperational:true` | Unhandled error when soft-deleting a request that doesn't exist or has invalid status |

> Note: `GET /api/admin/userStatuses/:id` (previously #2) was **removed from the suite** — the `UserStatuses` table no longer exists in the DB; it was replaced by `UserRoleStatusCurrent` (`statusId → Statuses`, `userRoleId → UserRole`). The dead endpoint is not tested anymore.
...
| # | Endpoint | Reason |
|---|----------|--------|
| 5 | `GET /api/user/verify-email` | Needs a real email verification token (generated during signup) |
| 6 | `GET /api/user/verify-phone` | Needs a real SMS verification code |
| 7 | `POST /api/user/verify-phone` | Needs a real SMS verification code |
...
| 10 | `GET /api/user/report-wrong-email` | Uses `?token=` query param from email link, not auth header |
...
| 12 | `PUT /api/shipperRequest/markJourneyCompletionAsSeen` | No completed request to mark |
...
| 15 | `markNegativeStatusAsSeen` (runner) | No pending negative notification |
...
| 29 | `POST /api/finance/journeyPayments` | No existing `journeyDecision` or `paymentMethod` records |
| 30 | `POST /api/finance/userDeposit` | No valid `financialInstitutionAccount` FK |
| 31 | `POST /api/finance/userSubscription` | No valid `subscriptionPlanPricing` FK |
| 32 | `POST /api/notifications/send-to-user` | 400 — needs valid `userUniqueId` with FCM token |
...
|---|------|-------------------|
| 33 | `testCancelBatch` | Batch can't be canceled in current status state |
...
| 41 | `PUT /api/user/attachedDocuments/:id` | Endpoint requires `multipart/form-data`, test sends JSON |

**Fix approach:** These are expected behaviors, not test gaps. The multipart endpoint (#41) can be tested with `form-data` instead of JSON if desired. For company ratings, seed a `companyBidRequest` in test data.
...
|----------|-------|---------------|
| **P0 — Server bugs** | 4 | Fix controller/service code |
