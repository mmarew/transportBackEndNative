...
const ORIGIN = { latitude: 9.02047, longitude: 38.80240, description: "Megenagna, Addis Ababa" };
const DEST   = { latitude: 9.03000, longitude: 38.76000, description: "Bole, Addis Ababa" };
...
async function login(phoneNumber, roleId) {
  ...
  const r = await request("POST", "/api/user/verifyUserByOTP", {
    phoneNumber, OTP: DEFAULT_OTP, roleId,
  });
...
const s = {
  adminToken         : null,
  shipperToken       : null,  shipperUniqueId    : null,
  driverToken        : null,  driverUniqueId     : null,
...
async function run() {
  console.log("\n\x1b[1m╔══════════════════════════════════════════════════╗\x1b[0m");
  console.log(  "\x1b[1m║   Company Bid + Individual — Full E2E Test       ║\x1b[0m");
  ...
  try {
    ...
    await step("Driver login (existing account)", async () => {
      ...
      const r = await request("GET", "/api/driver/verifyDriverJourneyStatus", null, bearer(s.driverToken));
      s.driverUniqueId = r.body?.driver?.driver?.userUniqueId;
      assert(s.driverUniqueId, `No driverUniqueId: ${JSON.stringify(r.body).slice(0, 200)}`);
    ...
    await step("Shipper: Register + login", async () => {
      const reg = await request("POST", "/api/user/createUser", {
        ...
        fullName: `E2E Shipper ${runId}`, email: `shipper_${runId}@test.com`,
      });
    ...
    await step("Fetch vehicle type", async () => {
      const r = await request("GET", "/api/admin/vehicleTypes?limit=1", null, bearer(s.adminToken));
    ...
    await step("Company: Register user, create company, login", async () => {
      ...
      const reg = await request("POST", "/api/user/createUser", {
        ...
        fullName: `E2E Company ${runId}`, email: `company_${runId}@test.com`,
      });
      assert(reg.body?.message === "success", `Company user register: ${JSON.stringify(reg.body)}`);
      ...
      assert(s.companyUniqueId, `No companyUniqueId in: ${JSON.stringify(create.body).slice(0, 300)}`);
      return `companyUniqueId: ${s.companyUniqueId}`;
    });

    await step("Company: Register driver's vehicle to company fleet", async () => {
      ...
      assert(s.vehicleUniqueId, `No vehicleUniqueId in driver status: ${JSON.stringify(r.body?.driver?.vehicle)}`);
    ...
    await step("Admin: Approve company (required before bidding)", async () => {
      ...
      assert(r.body?.message === "success", `Approve company: ${JSON.stringify(r.body).slice(0, 300)}`);
    ...
    await step("Company: Auto-assign drivers to slots", async () => {
      ...
      assert(assigned > 0, `Auto-assign assigned 0 slots. Fleet may not be registered. Detail: ${JSON.stringify(r.body?.data)}`);
    ...
    await step("Verify: DriverRequest status = 2 (requested)", async () => {
      ...
      assert(status === 2, `Expected journeyStatusId=2, got=${status}. Body: ${JSON.stringify(r.body?.driver?.driver)}`);
      return `journeyStatusId: ${status} ✓`;
    });

    await step("Driver: Confirm assignment (status 2→4)", async () => {
      assert(s.assignmentUniqueId, "No assignmentUniqueId from auto-assign step");
      const r = await request(
        "PATCH", `/api/company/assignments/${s.assignmentUniqueId}/status`,
        {
      ...
      assert(r.body?.message === "success", `Confirm: ${JSON.stringify(r.body)}`);
    ...
    await step("Driver: Start journey (goods loaded, status→5)", async () => {
      ...
      assert(r.body?.message === "success", `Start journey: ${JSON.stringify(r.body)}`);
    ...
    await step("Driver: Complete delivery (status→6)", async () => {
      ...
      assert(r.body?.message === "success", `Complete: ${JSON.stringify(r.body)}`);
    ...
    await step("Shipper: Create individual request", async () => {
      ...
      assert(r.body?.message === "success", `Create individual request: ${JSON.stringify(r.body)}`);
    ...
    await step("Fetch individual passengerRequestUniqueId", async () => {
      ...
      const rows = (r.body?.formattedData || []).map((f) => f.passengerRequest || f);
      const match = rows.find((x) => x.passengerRequestBatchId === s.indiBatchId) || rows[0];
      s.indiPassengerRequestId = match?.passengerRequestUniqueId;
      assert(s.indiPassengerRequestId, `No passengerRequestUniqueId: ${JSON.stringify(r.body).slice(0, 200)}`);
    ...
    await step("Driver: Accept individual request (status 1→3)", async () => {
      ...
      assert(r.body?.message === "success", `Accept: ${JSON.stringify(r.body).slice(0, 300)}`);
    ...
    await step("Fetch journey decision IDs (status=3)", async () => {
      ...
      s.indiDriverRequestId   = driverReq.driverRequestUniqueId || decision.driverRequestUniqueId;
      assert(s.indiJourneyDecisionId, `No journeyDecisionUniqueId: ${JSON.stringify(r.body).slice(0, 300)}`);
    ...
    await step("Shipper: Accept driver bid (status→4)", async () => {
      ...
      assert(r.body?.message === "success", `Accept driver: ${JSON.stringify(r.body).slice(0, 300)}`);
    ...
    await step("Driver: Start individual journey (status→5)", async () => {
      ...
      assert(r.body?.message === "success", `Start: ${JSON.stringify(r.body).slice(0, 300)}`);
      s.indiJourneyUniqueId = r.body?.uniqueIds?.journeyUniqueId || r.body?.journey?.journeyUniqueId;
    ...
    await step("Driver: Complete individual journey (status→6)", async () => {
      ...
      assert(r.body?.message === "success", `Complete: ${JSON.stringify(r.body).slice(0, 300)}`);
