...
async function runE2E() {
  ...
  try {
    ...
    const [shippers] = await dbClient.query(`
        ...
        `);
    if (!shippers.length) {
      throw new Error("No Shipper (Role 1) found.");
    }
    ...
    const [companies] = await dbClient.query(`
        ...
        `);
    if (!companies.length) {
      throw new Error("No approved Transport Company found.");
    }
    ...
    if (!dispatchers.length) {
      throw new Error("No Dispatcher/Member found for Company.");
    }
    ...
    if (rawDrivers.length < 2) {
      throw new Error(
        `Need at least 2 drivers with vehicles in the DB. Found ${rawDrivers.length}.`,
      );
    }

    // Forcefully ensure these 2 drivers belong to the company
    for (let drv of rawDrivers) {
      const [memCheck] = await dbClient.query(
        `SELECT * FROM CompanyMembership WHERE userUniqueId = ? AND companyUniqueId = ?`,
        [drv.userUniqueId, companyId],
      );
    ...
    // Generate Tokens
    const shipperToken = await generateToken(
      shipper.userUniqueId,
      shipper.phoneNumber,
      shipper.roleId,
    );
    const dispatcherToken = await generateToken(
      dispatcher.userUniqueId,
      dispatcher.phoneNumber,
      dispatcher.roleId,
    );
    const driver1Token = await generateToken(
      driver1.userUniqueId,
      driver1.phoneNumber,
      driver1.roleId,
    );
    const driver2Token = await generateToken(
      driver2.userUniqueId,
      driver2.phoneNumber,
      driver2.roleId,
    );
    ...
    let batchId;
    let shipperRequestArray;
    try {
      batchId = uuidv4();
      /* const reqRes = */ await shipperClient.post(
        "/shipperRequest/createRequest",
        {
      ...
      if (!shipperRequestArray || shipperRequestArray.length < 2) {
        console.log(getBatchRes.data);
        throw new Error("Batch did not create multiple shipper requests.");
      }
      console.log(
        `✅ Batch created: ${batchId} with ${shipperRequestArray.length} slots.`,
      );
    } catch (e) {
    ...
    try {
      ...
      const getAssignRes = await dispatcherClient.get(
        `/company/assignments?assignmentUniqueId=${assignment1UniqueId}`,
      );
      driverRequest1UniqueId = getAssignRes.data.data[0].driverRequestUniqueId;
      console.log(
        `✅ Driver 1 precisely assigned to Slot 1. (Assignment ID: ${assignment1UniqueId}, DriverReq: ${driverRequest1UniqueId})`,
      );
    } catch (e) {
    ...
    try {
      await driver1Client.put(
        "/driver/cancelDriverRequest?cancellationReasonsTypeId=1",
      );
      console.log(
        `✅ Driver 1 correctly rejected the journey. Assignment kicked to 'rejected_by_driver' or 'cancelled'.`,
      );
    } catch (e) {
    ...
    const verifyAssignRes = await dispatcherClient.get(
      `/company/assignments?assignmentUniqueId=${assignment1UniqueId}`,
    );
    ...
    try {
      ...
      const getAssignRes = await dispatcherClient.get(
        `/company/assignments?assignmentUniqueId=${assignment2UniqueId}`,
      );
    ...
    try {
      ...
      const confRes = await driver2Client.patch(
        `/company/assignments/${assignment2UniqueId}/status`,
        {
          assignmentStatus: "confirmed_by_driver",
          originLatitude: 8.9806,
      ...
      console.log(
        `✅ Driver 2 Confirmed. JourneyDecision: ${journeyDecisionUniqueId}`,
      );
    } catch (e) {
    ...
    const slot1 = shipperRequestArray[0].shipperRequest.shipperRequestUniqueId;
    ...
    try {
      ...
      console.log(
        `✅ Journey successfully Completed! Fleet returned to available pool.`,
      );
    } catch (e) {
      console.error(
        "Step 8/9 failed:",
