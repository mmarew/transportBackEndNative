...
async function getToken(phone, roleId) {
  ...
  if (loginRes.status !== 200) {
    console.log(`Login failed for ${phone}:`, loginRes.data);
  }
  ...
  if (res.status !== 200) {
    console.log(`Verify-OTP failed for ${phone}:`, res.data);
  }
  return {
...
(async () => {
  ...
  try {
    ...
    console.log(
      "  [DEBUG] VT Response Data:",
      JSON.stringify(vRes.data, null, 2),
    );
    ...
    const cRes = await request(
      ...
      { companyName: `CapCorp-${runId}`, companyPhone: DISPATCHER_PHONE },
      adminH(),
    );
    console.log("  [DEBUG] Company Create Status:", cRes.status);
    console.log(
      "  [DEBUG] Company Create Data:",
      JSON.stringify(cRes.data, null, 2),
    );
    ...
    await request(
      "POST",
      `/api/company/memberships/${state.dispatcherUniqueId}`,
      {
    ...
    for (let i = 1; i <= 2; i++) {
      ...
      const vr = await request(
        "POST",
        `/api/user/vehicles/driverUserUniqueId/${dId}`,
        {
          licensePlate: `CAP-${i}-${runId}`,
          color: "White",
      ...
      assert(
        vid,
        `Vehicle ${i} registration failed: ${JSON.stringify(vr.data)}`,
      );
    ...
    const br1 = await request(
      "POST",
      "/api/shipperRequest/createRequest",
      {
        ...commonReq,
        shipperRequestBatchId: state.batch1,
        numberOfVehicles: 2,
      },
      shipH(),
    );
    ...
    console.log(
      "  [DEBUG] Batch 1 Create Data:",
      JSON.stringify(br1.data, null, 2),
    );
    ...
    const br2 = await request(
      ...
      {
        ...commonReq,
        shipperRequestBatchId: state.batch2,
        numberOfVehicles: 1,
      },
      shipH(),
    );
    ...
    assert.strictEqual(
      bid1Res.status,
      201,
      `Bid 1 failed: ${JSON.stringify(bid1Res.data)}`,
    );
    ...
    assert.strictEqual(
      bid2Res.status,
      400,
      "Bid 2 should have failed with 400",
    );
    assert(
      JSON.stringify(bid2Res.data).toLowerCase().includes("capacity"),
    ...
    assert.strictEqual(
      bid2Retry.status,
      201,
      `Retry failed: ${JSON.stringify(bid2Retry.data)}`,
    );
