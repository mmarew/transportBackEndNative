...
async function runUXTest() {
  ...
  try {
    ...
    console.log(`  ✅ Using VehicleType: ${vtRes.data.data[0].vehicleTypeName} (${vtID})`);
    ...
    const vAssignRes = await request("POST", "/api/company/fleet", {
      companyUniqueId: cID,
      vehicleUniqueId: vID,
    ...
    const prID = prs.data.data[0].passengerRequestUniqueId;

    await request("POST", "/api/company/assignments", {
      companyBidRequestUniqueId: bidID,
      assignments: [{ passengerRequestUniqueId: prID, vehicleUniqueId: vID, driverUserUniqueId: drUID }]
    }, dH);
