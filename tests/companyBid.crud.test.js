...
const LAT_O = 9.03, LNG_O = 38.76, LAT_D = 9.04, LNG_D = 38.77;
...
(async () => {
  ...
  await step("Admin: migrate DB (CREATE IF NOT EXISTS all tables)", async () => {
  ...
  await step("Fetch a vehicleType from system", async () => {
    const res = await request("GET", "/api/admin/vehicleTypes?limit=5", null, adminH());
    const items = res.body?.data || [];
    assert(items.length > 0, "No vehicle types — run installPreDefinedData first");
  ...
  await step("Driver: create vehicle", async () => {
    const res = await request(
      "POST", "/api/user/vehicles/driverUserUniqueId/self",
      {
  ...
  await step("Approve company", async () => {
    const res = await request(
      "PATCH", `/api/company/companies/${state.companyUniqueId}/approve`,
      { approvalStatus: "approved", approvalReason: "All docs verified" },
      adminH(),
    );
    assert(res.body?.message === "success", `Approve failed: ${JSON.stringify(res.body)}`);
    return "Approved";
  });
  ...
  await step("Verify company is now approved", async () => {
    ...
    assert(co?.approvalStatus === "approved", `Expected approved, got: ${co?.approvalStatus}`);
    return "Confirmed approved";
  });

  await step("Approve non-existent company → 404", async () => {
    const res = await request(
      "PATCH", `/api/company/companies/00000000-0000-4000-8000-000000000001/approve`,
      { approvalStatus: "approved" },
      adminH(),
    );
    assert(res.status >= 400, `Expected error, got ${res.status}: ${JSON.stringify(res.body)}`);
  ...
  await step("Add driver as company member (dispatcher role)", async () => {
    ...
    assert(res.body?.message === "success", `Add member failed: ${JSON.stringify(res.body)}`);
  ...
  await step("Get members — verify driver is listed", async () => {
    const res = await request(
      "GET", `/api/company/memberships?companyUniqueId=${state.companyUniqueId}`,
      null, adminH(),
    );
    assert(res.body?.message === "success", `Get members failed: ${JSON.stringify(res.body)}`);
    const found = res.body?.data?.find((m) => m.userUniqueId === state.driverUniqueId);
  ...
  await step("Deactivate membership", async () => {
    const res = await request(
      "PATCH", `/api/company/memberships/${state.membershipUniqueId}/deactivate`,
      null, adminH(),
    );
    assert(res.body?.message === "success", `Deactivate failed: ${JSON.stringify(res.body)}`);
    return "Deactivated";
  });
  ...
  await step("Assign vehicle to company fleet", async () => {
    ...
    assert(res.body?.message === "success", `Assign vehicle failed: ${JSON.stringify(res.body)}`);
  ...
  await step("Get fleet — verify vehicle is listed", async () => {
    const res = await request(
      "GET", `/api/company/fleet?companyUniqueId=${state.companyUniqueId}`,
      null, adminH(),
    );
    assert(res.body?.message === "success", `Get fleet failed: ${JSON.stringify(res.body)}`);
    const found = res.body?.data?.find((v) => v.vehicleUniqueId === state.vehicleUniqueId);
  ...
  await step("Shipper: create batch request targeting company", async () => {
    ...
    const res = await request(
      "POST", "/api/passengerRequest/createRequest",
      {
        ...
        destination: { latitude: LAT_D, longitude: LNG_D, description: "Kazanchis" },
    ...
    assert(res.body?.message === "success", `Create request failed: ${JSON.stringify(res.body)}`);
    return `batchId: ${state.passengerRequestBatchId}`;
  });

  await step("Fetch passengerRequestUniqueId for batch", async () => {
    const res = await request(
      "GET",
    ...
    const rows = formatted
      ...
      .filter(Boolean);
    const match =
      rows.find((r) => r.passengerRequestBatchId === state.passengerRequestBatchId) || rows[0];
    state.passengerRequestUniqueId = match?.passengerRequestUniqueId;
    assert(state.passengerRequestUniqueId, `No passengerRequestUniqueId found. rows: ${rows.length}`);
  ...
  await step("Company submits bid for full batch", async () => {
    ...
    assert(res.body?.message === "success", `Submit bid failed: ${JSON.stringify(res.body)}`);
  ...
  await step("Get bids — verify created", async () => {
    ...
    assert(res.body?.message === "success", `Get bids failed: ${JSON.stringify(res.body)}`);
    const bid = res.body?.data?.[0];
    assert(bid, "Bid not found");
    assert(bid.bidStatus === "submitted", `Expected submitted, got: ${bid.bidStatus}`);
  ...
  await step("Shipper accepts bid (status → accepted_by_shipper)", async () => {
    ...
    assert(res.body?.message === "success", `Accept bid failed: ${JSON.stringify(res.body)}`);
  ...
  await step("Dispatcher assigns vehicle + driver to request slot", async () => {
    const res = await request(
    ...
    assert(res.body?.message === "success", `Create assignment failed: ${JSON.stringify(res.body)}`);
    ...
    assert(state.driverRequestUniqueId, "No driverRequestUniqueId — DriverRequest not auto-created");
  ...
  await step("Get assignments — verify slot listed", async () => {
    ...
    assert(res.body?.message === "success", `Get assignments failed: ${JSON.stringify(res.body)}`);
    const a = res.body?.data?.[0];
    assert(a, "Assignment not found");
    assert(a.assignmentStatus === "assigned", `Expected assigned, got: ${a.assignmentStatus}`);
    assert(a.driverRequestUniqueId, "driverRequestUniqueId missing from assignment record");
  ...
  await step("Driver confirms assignment → auto-creates JourneyDecision", async () => {
    const res = await request(
      "PATCH",
    ...
    assert(res.body?.message === "success", `Confirm failed: ${JSON.stringify(res.body)}`);
  ...
  await step("Verify assignment status = confirmed_by_driver", async () => {
    ...
    const a = res.body?.data?.[0];
    assert(a?.assignmentStatus === "confirmed_by_driver",
      `Expected confirmed_by_driver, got: ${a?.assignmentStatus}`);
    assert(a?.journeyDecisionUniqueId === state.journeyDecisionUniqueId,
  ...
  await step("Delete assignment", async () => {
    const res = await request(
      "DELETE", `/api/company/assignments/${state.assignmentUniqueId}`,
      null, adminH(),
    );
    assert(res.body?.message === "success", `Delete assignment failed: ${JSON.stringify(res.body)}`);
    return "Deleted";
  });
  ...
  await step("Delete bid", async () => {
    const res = await request(
      "DELETE", `/api/company/bids/${state.companyBidRequestUniqueId}`,
      null, adminH(),
    );
    assert(res.body?.message === "success", `Delete bid failed: ${JSON.stringify(res.body)}`);
    return "Deleted";
  });

  await step("Remove vehicle from fleet", async () => {
    const res = await request(
      "DELETE", `/api/company/fleet/${state.companyVehicleUniqueId}`,
      null, adminH(),
    );
    assert(res.body?.message === "success", `Remove vehicle failed: ${JSON.stringify(res.body)}`);
    return "Removed";
  });

  await step("Delete membership", async () => {
    const res = await request(
      "DELETE", `/api/company/memberships/${state.membershipUniqueId}`,
      null, adminH(),
    );
    assert(res.body?.message === "success", `Delete membership failed: ${JSON.stringify(res.body)}`);
    return "Deleted";
  });

  await step("Delete company", async () => {
    const res = await request(
      "DELETE", `/api/company/companies/${state.companyUniqueId}`,
      null, adminH(),
    );
    assert(res.body?.message === "success", `Delete company failed: ${JSON.stringify(res.body)}`);
    return "Deleted";
  });
