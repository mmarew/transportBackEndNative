...
async function getToken(phone, roleId) {
  ...
  console.log(`  [DEBUG] Login (${phone}, role ${roleId}) -> Status: ${loginRes.status}`);
  if (loginRes.status !== 200) {console.log("    [DEBUG] Login Data:", JSON.stringify(loginRes.data, null, 2));}
  ...
  if (res.status !== 200) {console.log("    [DEBUG] Verify Data:", JSON.stringify(res.data, null, 2));}
...
(async () => {
  ...
  await request("POST", "/api/user/createUser", { phoneNumber: dPhone, roleId: 7, fullName: "Consistency Dispatcher" });
  ...
  const compRes = await request("POST", "/api/company/companies", { companyName: `ConsistCorp-${runId}`, companyPhone: dPhone }, adminH);
  console.log("  [DEBUG] Company Create Status:", compRes.status);
  if (compRes.status !== 201) {
    console.log("  [DEBUG] Company Create Data:", JSON.stringify(compRes.data, null, 2));
  }
  ...
  await request("PATCH", `/api/company/companies/${compId}/approve`, { approvalStatus: "approved" }, adminH);
    
  // Membership
  await request("POST", `/api/company/memberships/${dispId}`, {
  ...
  const vr = await request("POST", `/api/user/vehicles/driverUserUniqueId/${dispId}`, { licensePlate: `CON-${runId}`, color: "Silver", vehicleTypeUniqueId: vtid }, adminH);
  ...
  assert(finalPrId, "Failed to create/find passenger request");
  ...
  await pool.query("UPDATE PassengerRequest SET journeyStatusId = 4 WHERE passengerRequestUniqueId = ?", [finalPrId]);
  ...
  const acceptRes = await request("PATCH", `/api/company/bids/${bidId}/status`, { bidStatus: "accepted_by_shipper" }, adminH);
    
  console.log("  [DEBUG] Accept Status:", acceptRes.status);
  console.log("  [DEBUG] Accept Error:", JSON.stringify(acceptRes.data, null, 2));

  assert.strictEqual(acceptRes.status, 409, "Should fail with 409 Conflict");
  assert(JSON.stringify(acceptRes.data).includes("Consistency Conflict"), "Error should mention Consistency Conflict");
  console.log("  ✅ PASS — System correctly detected the 'Hijack' and blocked the company bid.");
