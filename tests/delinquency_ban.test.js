...
async function getToken(phone, roleId) {
  ...
  const r = await request("POST", "/api/user/verifyUserByOTP", { phoneNumber: phone, OTP: DEFAULT_OTP, roleId });
...
(async () => {
  ...
  await step("Admin: migrate DB (CREATE IF NOT EXISTS)", async () => {
    ...
    assert(r.body?.message === "success" || r.status === 200, `DB migrate failed: ${JSON.stringify(r.body)}`);
    return "Tables verified";
  });

  await step("Admin: install predefined data (seed delinquency types)", async () => {
    const r = await request("POST", "/api/admin/installPreDefinedData", null, adminH());
  ...
  await step("Fetch driver delinquency type UUID", async () => {
    const r = await request("GET", "/api/admin/delinquency-types?limit=50", null, adminH());
    const types = r.body?.data || [];
    const t = types.find((d) => d.delinquencyTypeName?.toLowerCase().includes("driver commission"));
    assert(t, `Driver delinquency type not found. Run installPreDefinedData. Got: ${JSON.stringify(types.map(d=>d.delinquencyTypeName))}`);
  ...
  await step("Fetch company delinquency type UUID", async () => {
    const r = await request("GET", "/api/admin/delinquency-types?limit=50", null, adminH());
    const types = r.body?.data || [];
    const t = types.find((d) => d.delinquencyTypeName?.toLowerCase().includes("company commission"));
    assert(t, `Company delinquency type not found. Got: ${JSON.stringify(types.map(d=>d.delinquencyTypeName))}`);
  ...
  await step("Register driver", async () => {
    ...
    assert(r.body?.message === "success", `Register failed: ${JSON.stringify(r.body)}`);
  ...
  await step("Create transport company", async () => {
    const r = await request("POST", "/api/company/companies", {
    ...
    assert(r.body?.message === "success", `Create company failed: ${JSON.stringify(r.body)}`);
  ...
  await step("Create driver delinquency", async () => {
    const r = await request("POST", "/api/admin/user-delinquency", {
    ...
    assert(r.body?.message === "success", `Create delinquency failed: ${JSON.stringify(r.body)}`);
    s.userDelinquencyUniqueId = r.body?.userDelinquencyUniqueId || r.body?.data?.userDelinquencyUniqueId;
    assert(s.userDelinquencyUniqueId, `No userDelinquencyUniqueId in: ${JSON.stringify(r.body)}`);
  ...
  await step("Get driver delinquencies — verify created", async () => {
    const r = await request("GET",
      `/api/admin/getDelinquencyByFilter?userUniqueId=${s.driverUniqueId}&roleId=2`, null, adminH());
    assert(r.body?.message === "success", `Get failed: ${JSON.stringify(r.body)}`);
    const found = r.body?.data?.find((d) => d.userDelinquencyUniqueId === s.userDelinquencyUniqueId);
  ...
  await step("Get delinquency list for driver — verify count", async () => {
    const r = await request("GET",
      `/api/admin/getDelinquencyByFilter?userUniqueId=${s.driverUniqueId}&roleId=2`, null, adminH());
    assert(r.body?.message === "success", `List failed: ${JSON.stringify(r.body)}`);
  ...
  await step("Update driver delinquency description", async () => {
    const r = await request("PUT", `/api/admin/user-delinquency/${s.userDelinquencyUniqueId}`,
      { delinquencyDescription: "Updated description" }, adminH());
    assert(r.body?.message === "success", `Update failed: ${JSON.stringify(r.body)}`);
    return "Updated";
  });

  await step("Delete delinquency with no linked ban → success (skipDuplicate=true record)", async () => {
    // We will re-create one to test deletion since auto-ban may have been created
    const r2 = await request("POST", "/api/admin/user-delinquency", {
      userUniqueId: s.driverUniqueId,
      roleId: 2,
      delinquencyTypeUniqueId: s.driverDelinquencyTypeUniqueId,
    ...
    if (r2.body?.automaticAction?.action === "none") {
      const rd = await request("DELETE", `/api/admin/user-delinquency/${tempId}`, null, adminH());
      assert(rd.body?.message === "success", `Delete failed: ${JSON.stringify(rd.body)}`);
  ...
  await step("Manual ban driver (admin-initiated via userRoleUniqueId)", async () => {
    ...
    const rb = await request("GET", `/api/admin/banned-users?isActive=true`, null, adminH());
    const ban = rb.body?.data?.find((b) => b.driverUniqueId === s.driverUniqueId ||
  ...
  await step("Get banned users — verify driver in list", async () => {
    const r = await request("GET", `/api/admin/banned-users?roleId=2&isActive=true`, null, adminH());
    assert(r.body?.message === "success", `Get banned users failed: ${JSON.stringify(r.body)}`);
  ...
  await step("Create company delinquency", async () => {
    const r = await request("POST", "/api/company/admin/delinquency", {
    ...
    assert(r.body?.message === "success", `Create company delinquency failed: ${JSON.stringify(r.body)}`);
    s.companyDelinquencyUniqueId = r.body?.companyDelinquencyUniqueId || r.body?.data?.companyDelinquencyUniqueId;
  ...
  await step("Get company delinquencies — verify created", async () => {
    const r = await request("GET",
      `/api/company/admin/delinquency?companyUniqueId=${s.companyUniqueId}`, null, adminH());
    assert(r.body?.message === "success", `Get failed: ${JSON.stringify(r.body)}`);
  ...
  await step("Get company bans — verify auto-ban created", async () => {
    const r = await request("GET",
      `/api/company/admin/delinquency/bans?companyUniqueId=${s.companyUniqueId}&isActive=true`, null, adminH());
  ...
  await step("Manual ban company (if no auto-ban)", async () => {
    ...
    assert(r.body?.message === "success", `Manual ban failed: ${JSON.stringify(r.body)}`);
  ...
  await step("Get company bans with isActive=false — none expected", async () => {
    const r = await request("GET",
      `/api/company/admin/delinquency/bans?companyUniqueId=${s.companyUniqueId}&isActive=false`, null, adminH());
    assert(r.body?.message === "success", `Get failed: ${JSON.stringify(r.body)}`);
  ...
  await step("Unban non-existent company ban → 404", async () => {
    const r = await request("PATCH",
      `/api/company/admin/delinquency/bans/00000000-0000-4000-8000-000000000002/unban`, null, adminH());
    assert(r.status >= 400, `Expected error, got ${r.status}`);
    return `Correctly rejected (${r.status})`;
  });
