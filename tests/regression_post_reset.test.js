...
async function activateRole(phone, roleId) {
  ...
  const [rows] = await pool.query(`
        ...
        WHERE u.phoneNumber = ? AND ur.roleId = ?`, [phone, roleId]);
    
  if (rows.length > 0) {
    const { userRoleId, userUniqueId } = rows[0];
    console.log(`  [DB] Activating Role ${roleId} for ${phone} (URID: ${userRoleId})`);
        
    // Clean up any existing statuses to avoid duplicates since userRoleId isn't unique in this table
    await pool.query("DELETE FROM UserRoleStatusCurrent WHERE userRoleId = ?", [userRoleId]);
    ...
    await pool.query(`
            ...
            VALUES (?, 1, ?, 'Active for E2E', ?, NOW())`, [crypto.randomUUID(), userRoleId, userUniqueId]);
  } else {
...
(async () => {
  ...
  try {
    ...
    assert.strictEqual(seedRes.status, 200, "Seeding failed: " + JSON.stringify(seedRes.data));
    console.log("  ✅ Data Seeded");

    // [04] Setup Shipper, Dispatcher & Driver
    console.log("  [04] Setup Shipper, Dispatcher & Driver ...");
    // Shipper
    await request("POST", "/api/user/createUser", { phoneNumber: SHIPPER_PHONE, roleId: 1, fullName: "E2E Shipper" });
    ...
    // Dispatcher
    await request("POST", "/api/user/createUser", { phoneNumber: DISPATCHER_PHONE, roleId: 7, fullName: "E2E Dispatcher" });
    ...
    await request("POST", "/api/user/createUser", { phoneNumber: DRIVER_PHONE, roleId: 2, fullName: "E2E Driver" });
    ...
    // Company
    const cRes = await request("POST", "/api/company/companies", { companyName: `E2E-Corp-${runId}`, companyPhone: DISPATCHER_PHONE }, adminH);
    state.companyId = cRes.data.data.companyUniqueId;
    await request("PATCH", `/api/company/companies/${state.companyId}/approve`, { approvalStatus: "approved" }, adminH);
        
    // Memberships
    await request("POST", `/api/company/memberships/${state.dispatcherId}`, { 
      companyUniqueId: state.companyId, 
      companyRoleUniqueId: "750858d6-e816-45b0-a088-9dfe6b4d80ff" // Dispatcher
    }, adminH);
    await request("POST", `/api/company/memberships/${state.driverId}`, { 
      companyUniqueId: state.companyId, 
      companyRoleUniqueId: "240858d6-e816-45b0-a088-9dfe6b4d80ff" // Driver
    }, adminH);

    const vtRes = await request("GET", "/api/admin/vehicleTypes?limit=1", null, adminH);
    state.vehicleTypeId = vtRes.data.data[0].vehicleTypeUniqueId;

    // Register 1 Truck for the Driver
    const vr = await request("POST", `/api/user/vehicles/driverUserUniqueId/${state.driverId}`, { 
      licensePlate: `E2E-${runId}`, color: "Blue", vehicleTypeUniqueId: state.vehicleTypeId 
    }, adminH);
    state.vehicleUniqueId = vr.data.data.vehicleUniqueId;

    await request("POST", "/api/company/fleet", { 
      companyUniqueId: state.companyId, vehicleUniqueId: state.vehicleUniqueId, assignmentStartDate: new Date().toISOString() 
    }, adminH);
    ...
    const createBatch = async (name) => {
      ...
      const fetch = await request("GET", `/api/user/getPassengerRequest4allOrSingleUser?target=all&passengerRequestBatchId=${bId}`, null, shipH);
      return { bId, pId: fetch.data.formattedData[0].passengerRequest.passengerRequestUniqueId };
    };
    ...
    const bidRes = await request("POST", "/api/company/bids", {
      passengerRequestBatchId: state.batchId,
      companyUniqueId: state.companyId,
    ...
    await request("PATCH", `/api/company/bids/${state.bidId}/status`, { bidStatus: "accepted_by_shipper" }, adminH);
    ...
    const capRes = await request("POST", "/api/company/bids", {
      passengerRequestBatchId: state.batchIdB, // Use existing Batch B
      companyUniqueId: state.companyId,
      numberOfVehiclesOffered: 1,
      vehicleTypeUniqueId: state.vehicleTypeId,
      proposedTotalCost: 4500
    }, dispH);
    assert.strictEqual(capRes.status, 400, "Capacity should be reserved after bid acceptance");
    assert(JSON.stringify(capRes.data).includes("Fleet capacity exceeded"), "Error should be capacity exceeded");
    console.log("  ✅ Bid Accepted & Capacity Correctly Reserved");

    // [08] Assignment & Confirm
    console.log("  [08] Dispatcher assigns Driver ...");
    const asRes = await request("POST", "/api/company/assignments", {
      companyBidRequestUniqueId: state.bidId,
      passengerRequestUniqueId: state.prUniqueId,
    ...
    // Driver Confirm
    const confirmRes = await request("PATCH", `/api/company/assignments/${state.assignmentId}/status`, { assignmentStatus: "confirmed_by_driver" }, drivH);
    state.jdId = confirmRes.data.data.journeyDecisionUniqueId; // This one HAS a .data wrapper
    console.log("  ✅ Assignment Fixed & Driver Confirmed (JD: " + state.jdId + ")");

    // [09] Execution (Start -> Complete)
    console.log("  [09] Executing Journey ...");
    const startRes = await request("PUT", "/api/driver/startJourney", { 
    ...
    assert.strictEqual(startRes.status, 200, "Journey start failed: " + JSON.stringify(startRes.data));
    ...
    const completeRes = await request("PUT", "/api/driver/completeJourney", { 
      driverRequestUniqueId: state.drId,
      passengerRequestUniqueId: state.prUniqueId,
      journeyDecisionUniqueId: state.jdId,
    ...
    assert.strictEqual(completeRes.status, 200, "Journey completion failed: " + JSON.stringify(completeRes.data));
    console.log("  🏁 Journey Completed (Status 6)");

    // [10] Capacity Release Check
    console.log("  [10] Verifying Capacity Release ...");
    const finalCapCheck = await request("POST", "/api/company/bids", {
      passengerRequestBatchId: state.batchIdB, 
      companyUniqueId: state.companyId,
      numberOfVehiclesOffered: 1,
      vehicleTypeUniqueId: state.vehicleTypeId,
      proposedTotalCost: 4000
    }, dispH);
    assert.strictEqual(finalCapCheck.status, 201, "Capacity should be released after journey completion");
