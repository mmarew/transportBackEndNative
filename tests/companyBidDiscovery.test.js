...
(async () => {
  ...
  await step("Shipper: create TARGETED request", async () => {
    ...
    const res = await request(
      ...
      {
        shipperRequestBatchId: state.batchIdTargeted,
    ...
    assert(
      ...
      `Targeted Create failed: ${JSON.stringify(res.body)}`,
    );
  ...
  await step("Shipper: create OPEN request (no specific ID)", async () => {
    ...
    const res = await request(
      ...
      "/api/shipperRequest/createRequest",
      {
        shipperRequestBatchId: state.batchIdOpen,
        requestMode: "company_target",
    ...
    assert(
      res.body?.message === "success",
      `Open Create failed: ${JSON.stringify(res.body)}`,
    );
    return "Created open";
  });
  ...
  await step("Driver Discovery: verify both exist", async () => {
    ...
    const hasTargeted = data.some(
      (b) => b.shipperRequestBatchId === state.batchIdTargeted,
    );
    const hasOpen = data.some(
      (b) => b.shipperRequestBatchId === state.batchIdOpen,
    );
  ...
  await step(
    "Discovery: verify OPEN request hidden, TARGETED remains",
    async () => {
      const res = await request(
        "GET",
        "/api/company/bids?target=available",
