...
(async () => {
  ...
  await step("Infrastructure: login & registration", async () => {
    ...
    await request("POST", "/api/user/createUser", { phoneNumber: SHIPPER_PHONE, roleId: 1, fullName: "BulkShipper" });
    state.shipperToken = (await getToken(SHIPPER_PHONE, 1)).token;
    
    await request("POST", "/api/user/createUser", { phoneNumber: DRIVER_PHONE, roleId: 2, fullName: "BulkDriver1" });
    const dAuth = await getToken(DRIVER_PHONE, 2);
    state.driverToken = dAuth.token;
    state.driverUniqueId = dAuth.userUniqueId;

    await request("POST", "/api/user/createUser", { phoneNumber: DRIVER2_PHONE, roleId: 2, fullName: "BulkDriver2" });
  ...
  await step("Setup Company & Fleet", async () => {
    ...
    await request("POST", `/api/company/memberships/${state.driverUniqueId}`, {
      companyUniqueId: state.companyUniqueId,
      companyRoleUniqueId: DISPATCHER_ROLE_UUID,
      membershipStartDate: new Date().toISOString(),
    }, adminH());

    await request("POST", `/api/company/memberships/${state.driver2UniqueId}`, {
      companyUniqueId: state.companyUniqueId,
      companyRoleUniqueId: DISPATCHER_ROLE_UUID,
      membershipStartDate: new Date().toISOString(),
    }, adminH());
    ...
    // Assign to fleet
    await request("POST", "/api/company/fleet", { companyUniqueId: state.companyUniqueId, vehicleUniqueId: state.v1, assignmentStartDate: new Date().toISOString() }, adminH());
    await request("POST", "/api/company/fleet", { companyUniqueId: state.companyUniqueId, vehicleUniqueId: state.v2, assignmentStartDate: new Date().toISOString() }, adminH());
  ...
  await step("Shipper: Create 2-truck Batch", async () => {
    ...
    const reqData = {
      ...
      originLocation: { latitude: 9, longitude: 38, description: "Bulk Origin" },
    ...
    await request("POST", "/api/passengerRequest/createRequest", reqData, shipperH());
    await request("POST", "/api/passengerRequest/createRequest", reqData, shipperH());
    
    // Robust Lookup: Get the IDs from the database
    const lookup = await request("GET", `/api/user/getPassengerRequest4allOrSingleUser?target=all&passengerRequestBatchId=${state.batchId}`, null, adminH());
    assert(lookup.body?.formattedData?.length === 2, `Expected 2 requests in batch, found ${lookup.body?.formattedData?.length}`);
    state.pr1 = lookup.body.formattedData[0].passengerRequest.passengerRequestUniqueId;
    state.pr2 = lookup.body.formattedData[1].passengerRequest.passengerRequestUniqueId;
  ...
  await step("Company: Bid & Win", async () => {
    ...
    await request("PATCH", `/api/company/bids/${state.bidUniqueId}/status`, { bidStatus: "accepted_by_shipper" }, adminH());
    return "Bid Accepted";
  });

  await step("Dispatcher: PERFORM BULK ASSIGNMENT", async () => {
    const bulkRes = await request("POST", "/api/company/assignments/bulk", {
      companyBidRequestUniqueId: state.bidUniqueId,
      assignments: [
        { passengerRequestUniqueId: state.pr1, vehicleUniqueId: state.v1, driverUserUniqueId: state.driverUniqueId },
        { passengerRequestUniqueId: state.pr2, vehicleUniqueId: state.v2, driverUserUniqueId: state.driver2UniqueId }
      ]
    }, driverH());

    assert(bulkRes.status === 201, `Bulk status ${bulkRes.status}: ${JSON.stringify(bulkRes.body)}`);
    assert(bulkRes.body.data.length === 2, "Should have 2 results");
    return "Bulk Success ✓";
  });

  await step("Verify: Assignments in DB", async () => {
    const list = await request("GET", `/api/company/assignments?companyBidRequestUniqueId=${state.bidUniqueId}`, null, driverH());
