...
const createVehicleDriverController = async (req, res) => {
  try {
    ...
    return ServerResponder(res, { message: "error", error: "Unable to create vehicle-driver" }, 500);
  }
};
...
const getVehicleDriversController = async (req, res) => {
  try {
    const result = await getVehicleDrivers(req.query || {});
    return ServerResponder(res, result);
  } catch (error) {
    console.error("@getVehicleDriversController", error);
    return ServerResponder(res, { message: "error", error: "Unable to get vehicle-driver list" }, 500);
  }
};
...
const updateVehicleDriverController = async (req, res) => {
  try {
    ...
    return ServerResponder(res, { message: "error", error: "Unable to update vehicle-driver" }, 500);
  }
};
...
const deleteVehicleDriverController = async (req, res) => {
  try {
    ...
    return ServerResponder(res, { message: "error", error: "Unable to delete vehicle-driver" }, 500);
  }
};
