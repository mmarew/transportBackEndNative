...
**Dynamics Route Technology Solutions** is a startup technology company based in Ethiopia, focused on building digital infrastructure for the logistics and freight transportation industry. The company develops and operates the Dynamics Route platform, a multi-application ecosystem connecting cargo shippers, transport drivers, freight companies, and fleet administrators.

Founded with the mission to modernize Ethiopia's logistics sector, Dynamics Route provides a comprehensive suite of applications:

| Application           | Type   | Purpose                                                      |
| --------------------- | ------ | ------------------------------------------------------------ |
| **Dynamics Driver**   | Mobile | Transport drivers find, accept, and complete cargo jobs      |
...
As a startup preparing to integrate financial payment processing, Dynamics Route is committed to establishing a strong security foundation from the outset. This audit is a critical step toward ensuring the platform can securely handle financial transactions and sensitive user data in compliance with Ethiopian regulations and international security standards.
...
This document serves as Dynamics Route Technology Solutions' formal response to the Information Network Security Administration (INSA) Mobile Application Security Testing Requirements. It provides comprehensive documentation of the platform's architecture, data flows, security controls, and compliance posture as requested by INSA's Cyber Security Audit Division.

The response is structured to address each requirement outlined in INSA's Mobile Application Security Testing Requirements Document, covering:

- Business architecture and system design documentation
- Security controls and threat mitigation measures
- Code-level remediation of identified vulnerabilities
- Compliance mapping to industry standards

### 2.2 Objective of This Security Audit Request
...
The company recognizes that secure handling of user data, financial information, and transport operations is critical to its mission and is committed to achieving and maintaining a robust security posture.
...
flowchart TB
    ...
    subgraph "Client Layer"
        DRIVER["Driver App<br/>(React Native)"]:::client
        SHIPPER["Shipper App<br/>(React Native)"]:::client
        COMPANY["Company Web App<br/>(React + Vite)"]:::client
        ADMIN["Admin Panel<br/>(React + MUI)"]:::client
    end

    subgraph "Gateway"
        NGINX["Nginx / Vercel<br/>Reverse Proxy"]:::gateway
    end

    subgraph "Service Layer"
        API["Express.js API<br/>Port 3000-3002"]:::service
        WS["Socket.IO Server<br/>Redis Adapter"]:::service
        AUTH["Auth Service<br/>JWT + OTP"]:::service
        PAY["SantimPay<br/>Payment Service"]:::service
        TIMEOUT["Auto Timeout<br/>Detection"]:::service
    end

    subgraph "Data Layer"
        MYSQL[("MySQL 8<br/>70+ Tables")]:::data
        REDIS[("Redis / Upstash<br/>Cache + Pub/Sub")]:::data
        FS["Local Filesystem<br/>/uploads/"]:::data
    end

    subgraph "External Services"
        FCM["Firebase FCM<br/>Push Notifications"]:::ext
        SMS["AfroMessage<br/>SMS Gateway"]:::ext
        SMTP["SMTP Nodemailer"]:::ext
        SPAY["SantimPay<br/>Payment Gateway"]:::ext
        OSM["OpenStreetMap<br/>Nominatim + OSRM"]:::ext
    end
...
The following test accounts have been provisioned in the production database for the audit:

| Account Type      | Phone Number    | OTP                         | Role ID | Permissions                                              |
| ----------------- | --------------- | --------------------------- | ------- | -------------------------------------------------------- |
| **Super Admin**   | `+251983222221` | Provided via secure channel | 6       | Full system access, user management, configuration       |
| **Admin**         | `+251993333333` | Provided via secure channel | 3       | User management, document approval, dispute handling     |
| **Driver**        | `+251991111112` | Provided via secure channel | 2       | Job acceptance, journey management, vehicle registration |
| **Shipper**       | `+251992222222` | Provided via secure channel | 1       | Cargo request creation, driver selection, tracking       |
| **Company Admin** | `+251994444444` | Provided via secure channel | 7       | Company management, fleet, bidding                       |

> **Note:** One-time OTP values will be delivered separately via INSA's Audit Request Portal upon audit commencement. The accounts are active and ready for testing.
...
**Delivery Method for Source Code & Build Files:**
...
- WebSocket authentication via Socket.IO `auth` handshake option (token passed in `auth.token`, not in URL)
...
- API Key (X-API-Key): Delivered via INSA Audit Request Portal
- Firebase Service Account: Delivered via INSA Audit Request Portal
- SantimPay Merchant ID: Available in environment configuration (test mode)
...
| 2   | `rejectUnauthorized: false` in WebSocket  | Removed from all socket.io clients (Driver + Shipper + Web apps)                                  | ✅ Fixed                                                                          |
| 3   | `Math.random()` for OTP generation        | Replaced with`crypto.randomInt(100000, 999999)`                                                   | ✅ Fixed                                                                          |
| 4   | JWT token in WebSocket URL query param    | Moved to Socket.IO`auth` handshake option across all apps                                         | ✅ Fixed                                                                          |
...
| 19  | OAuth client secret in source code        | Moved from hardcoded`Key.js` (shipper + Driver) to environment variables                          | ✅ Fixed                                                                          |
| 20  | Internal IPs hardcoded in source          | Replaced with env-backed HTTPS URLs across all repos                                              | ✅ Fixed                                                                          |
| 21  | WebSocket auth scoping bug (Driver)       | `phoneNumber`/`token` out of scope in `initSocket`; now passed via `getSocketCredentials()`       | ✅ Fixed                                                                          |
| 22  | Hermes disabled on shipper app            | Shipper used JSC (Hermes commented out);`hermesEnabled=true` set in `build.gradle`                | ✅ Fixed                                                                          |
| 23  | Android cleartext traffic (shipper)       | `usesCleartextTraffic=true`; disabled + `network_security_config.xml` added                       | ✅ Fixed                                                                          |
| 24  | OTP stored in plaintext in DB             | `otpPlain` column removed from `login.service.js`; dev endpoint no longer exposes OTP             | ✅ Fixed                                                                          |
| 25  | Super admin fallback password`123456`     | Hardcoded fallback in`Config.js` removed — must be set via env var only                           | ✅ Fixed                                                                          |
| 26  | Backend Validator.js logs full body       | `console.error` printed full request body on validation failure; moved to sanitized `logger.warn` | ✅ Fixed                                                                          |
| 27  | Personal emails & internal IP (Driver)    | Fallback emails`mmarew@gmail.com`/`abatemarew@gmail.com` + IP `31.97.179.144` removed             | ✅ Fixed                                                                          |
| 28  | Self-hosted Nominatim HTTP-only (shipper) | Hardcoded`http://31.97.179.144:8081`; moved to env var with HTTPS default                         | ✅ Fixed                                                                          |
...
| 36  | Hardcoded IP in Postman collection        | `http://31.97.179.144:8081` replaced with example domain placeholder                              | ✅ Fixed                                                                          |
...
| 41  | Web app JWT in long-lived `localStorage`  | Web apps (Company, Admin, Queue Admin) migrated to httpOnly session cookie: JWT set as a SameSite=Lax, 24h, `httpOnly` cookie (Secure in prod); JWT no longer reachable from JS storage — only a non-secret user profile is cached locally; server supports dual-mode (cookie for web, `Authorization: Bearer` for mobile); CSRF mitigated via SameSite=Lax + Origin/Referer allowlist; logout clears the cookie server-side | ✅ Fixed |
...
| 2   | JWT httpOnly cookies for web apps              | Implemented — web auth now uses httpOnly SameSite=Lax cookies (24h, Secure in prod); JWT removed from JS storage on all web apps |
...
| --------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------- |
| **Authentication & Session Management** | JWT expiration (24h), rate limiting on auth routes (5 req/15min), generic login errors, OTP hardened with`crypto.randomInt()`, dual-mode auth (mobile: Keychain/Keystore Bearer tokens; web: httpOnly SameSite=Lax session cookie with CSRF Origin/Referer check, non-secret profile only in JS storage, server-side logout clearing the cookie), WebSocket auth migrated from URL params to `auth` handshake | All                               |
...
| **transportCompany**       | Web App (React 19/Vite/TypeScript)    | 25/28 (89%)       | ✅ SECURE — httpOnly SameSite=Lax session cookie (JWT removed from storage), dual-mode Bearer kept off-web, CSP meta tag, Redux DevTools disabled, console stripped, error messages sanitized, file uploads validated, example data sanitized in docs                                                                                                        |
| **transportAdmin**         | Web App (React 18/Vite/MUI)           | 19/19 (100%)      | ✅ SECURE — httpOnly SameSite=Lax session cookie (JWT removed from storage), WebSocket auth handshake w/ cookie, CSP added, Redux DevTools disabled, console stripped, E2E config sanitized, HTTPS/WSS URLs, no hardcoded fallback contacts                                                                                             |
| **queadmin-frontend**      | Web App (React 18/Vite/TypeScript)    | —                 | ✅ SECURE — httpOnly SameSite=Lax session cookie (JWT removed from storage; legacy token entry purged, only non-secret profile cached), Socket.IO auth handshake w/ cookie `withCredentials`, `.env` untracked, CSP + secure build defaults                                                                                              |
| **Total**                  | **6 Codebases**                       | **140/144 (97%)** | ✅ All critical and high-severity items resolved; remaining 4 are non-blocking (git-history scrub #6, shipper legacy fallback #12, cloud-console API-key restrictions, dep-scan automation)                                                                                  |
...
| 6   | Rotate web-app httpOnly-cookie signing keys | Defense-in-depth against long-lived session key exposure          | 15 min |
...
- ~ **Defense-in-depth architecture** — rate limiting, CSP, HSTS, Helmet headers, CORS restrictions, input validation, sanitized error handling, structured logging, and secure token management (Keychain/Keystore on mobile, httpOnly SameSite=Lax session cookies with CSRF Origin/Referer checks on web)

The remaining items are non-blocking and do not affect the runtime security posture: (1) git-history scrub of historical `.env`/keystore commits (#6), (2) shipper's AsyncStorage fallback on legacy devices without Keychain (#12), and (3) cloud-console API key restrictions (Google Maps, Firebase, OAuth client ID binding).
...
**Certification:** I/We certify that the information provided in this document is accurate and complete to the best of my/our knowledge. I/We understand that providing false or misleading information may result in the rejection of this submission or other actions as determined by INSA.
