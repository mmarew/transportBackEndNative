...
| All drivers in line refuse                                                    | Order stays`requested` with no active offer; notify org admins; **auto-offer resumes on the next matching-type check-in** (`checkin` rescans pending `waiting` orders FIFO → `offerToDriver`) or on manual `dispatch` |
