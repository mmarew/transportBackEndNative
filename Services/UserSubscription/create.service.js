...
const createUserSubscription = async ({
  ...
  if (durationInDays === null || durationInDays === undefined || durationInDays <= 0) {
    if (effectiveFrom && effectiveTo) {
      ...
      throw new AppError("durationInDays is missing for this pricing configuration.", 400);
    }
  }
  ...
  if (isFree) {
    ...
    if (dataGrantedBefore) {
      throw new AppError("You have already registered for a free trial once.", 400);
    }
  }
  ...
  const sql = `
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `;
  const values = [userSubscriptionUniqueId, driverUniqueId, subscriptionPlanPricingUniqueId, savedStartDate ? savedStartDate : today, nextDate, userSubscriptionCreatedBy || driverUniqueId, currentDate()];
