...
exports.createCompanyRating = async ({
  ...
  if (!bid) {throw new AppError("Freight job not found", AppError.NOT_FOUND);}
  ...
  if (existing) {throw new AppError("A rating already exists for this job", AppError.CONFLICT);}
...
exports.updateCompanyRating = async (companyRatingUniqueId, { rating, comment, updatedBy }) => {
  ...
  if (!existing) {throw new AppError("Rating not found", AppError.NOT_FOUND);}
